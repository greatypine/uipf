...
function sql(){
	this.initCompant = function(){
		sql_table = $('#sql_table').datagrid({
		        url: content+"/sqlquery/querySqls",
		        fitColumns:true,
		        singleSelect:true,
		        rownumbers:true,
		        loadMsg:'正在加载，请稍后...',
		        ...
		        columns:[[
				        ...
				        {field:'islimit',title:'是否分页',width:"6%",align:'center',formatter:function(val,row){
				        	if(val){
				        ...
				        {field:'pagesize',title:'页码',width:"6%",align:'center',hidden:true},
				        {field:'pagenum',title:'分页数',width:"6%",align:'center',hidden:true},
				        {field:'input_params_name',title:'输入对象',width:"8%"},
				        {field:'out_params_name',title:'输出对象',width:"8%"},
	...
	this.complate = function(){
		$('#sqldlg-fm').form('submit',{
			url:content+"/sqlquery/saveOrUpdate",
			onSubmit:function(){
