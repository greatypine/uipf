...
public class SchedulingCollection {
	...
	@Scheduled(cron = "10 00 22 * * ?") // 统计日渠道客户消费情况 10 00 22 * * ?
	...
	@Scheduled(cron = "10 10 00 01 * ?") // 统计日渠道客户消费情况 10 00 22 * * ?
    public String doCountMonthOrderChannelCost() {
    	...
    	if(timmerSwitchMapper.countByExample(example)>0) {
    		...
    		List<TSysUser> userlist = userMapper.queryQUserEmailAndPhone(map);
    		for (TSysUser tSysUser : userlist) {
    			String mess = getCountDayOrderChannelCostMess(tSysUser,"月客户消费统计","2");
    			if(mess!=null) emailService.sendHtmlMail(tSysUser.getEmail(), "痘卫士-月客户消费统计",mess);
    		}
