(function(b) {
	...
	a.config = {
		...
		tools: {
			...
			save: {
				onclick: function(c) {
					flowOption.saveWorkflow(eval("("+c+")"));
				}
			}
		},
		props: {
	...
	a.rect = function(p, m) {
		var _idx = a.util.nextId();
		var u = this,
			g = "rect" + _idx,
			E = b.extend(true, {}, a.config.rect, p),
			C = m,
			t, e, n, f, x, v;
		if(E!=undefined) E.text.text = E.text.text+_idx;
	...
	a.init = function(x, r) {
		...
		if (a.config.editable) {
			...
			b("#myflow_save").click(function() {
				...
				i += "},props:{props:{";
				for (var c in workflowdata.props.props) {
					i += c + ":{value:'" + workflowdata.props.props[c].value + "'},"
				}
				if (i.substring(i.length - 1, i.length) == ",") {
					i = i.substring(0, i.length - 1)
				}
